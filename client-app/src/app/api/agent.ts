...
import { history } from "../..";
import { Activity, ActivityFormValues, RandomActivity } from "../models/activity";
          ...
          case 400:
               ...
               if (data.errors) {
                    ...
                    for (const key in data.errors) {
                         if (data.errors[key]) {
                              modalStatusErrors.push(data.errors[key])
                              for (var i = 0; i < data.errors[key].length; i++) {

                                   toast.error(data.errors[key][i])
                              }
...
const RandomActivities = {
     get: () => requests.get<RandomActivity>('/BoredApi')
}
